...
public class TitaniumPoweredCrusherBlock extends AbstractPoweredCrusherBlock {
    ...
    public ActionResult onUse(BlockState state, World world, BlockPos pos,
        ...
        if (!world.isClient) {
            NamedScreenHandlerFactory screenHandlerFactory = ((TitaniumPoweredCrusherBlockEntity) world.getBlockEntity(pos));
    ...
    public BlockEntity createBlockEntity(BlockPos pos, BlockState state) {
        return new TitaniumPoweredCrusherBlockEntity(pos, state);
    }

    @Nullable
    @Override
    public <T extends BlockEntity> BlockEntityTicker<T> getTicker(World world, BlockState state, BlockEntityType<T> type) {
        return validateTicker(type, ModBlockEntities.TITANIUM_POWERED_CRUSHER,
